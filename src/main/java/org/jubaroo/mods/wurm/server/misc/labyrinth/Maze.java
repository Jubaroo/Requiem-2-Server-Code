...
import org.jubaroo.mods.wurm.server.misc.labyrinth.events.CreateMazeHedgeEvent;
import org.jubaroo.mods.wurm.server.misc.labyrinth.events.GrowMazeHedgeEvent;
import org.jubaroo.mods.wurm.server.misc.labyrinth.events.ShortEventDispatcher;
import org.jubaroo.mods.wurm.server.utils.StdRandom;
...
public class Maze {
            ...
            for (int x = 1; x <= this.mazeSize; ++x) {
                for (int yIter = 1; yIter <= this.mazeSize; ++yIter) {
                    ...
                    if (Server.rand.nextInt(this.mazeSize) == 1) {
                        SoundPlayer.playSound("sound.forest.branchsnap", x, y, true, 0f);
                    }
                }
            }
    ...
    public boolean createHedge(final Tiles.TileBorderDirection tbDir, final int x, final int y, final boolean animateGrowth) throws NoSuchZoneException, IOException {
        ...
        final DbFence fence = new DbFence(animateGrowth ? StructureConstantsEnum.HEDGE_FLOWER3_LOW : this.fenceType, startX + x, startY + y, 0, 1f, tbDir, zone.getId(), layer);
