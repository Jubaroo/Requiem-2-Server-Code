...
static String timers = "ObjectiveTimers";
    ...
    public static void onServerStarted() {
        ...
        try (Connection con = ModSupportDb.getModSupportDb()) {
            if (!ModSupportDb.hasTable(con, tableName)) {
                ...
                try (PreparedStatement ps = con.prepareStatement(String.format("INSERT INTO %s (ID, TIMER) VALUES(DEPOT, 0)", timers))) {
                    ps.executeUpdate();
                }
                try (PreparedStatement ps = con.prepareStatement(String.format("INSERT INTO %s (ID, TIMER) VALUES(TITAN, 0)", timers))) {
                    ps.executeUpdate();
                }
            }
        } catch (SQLException e) {
    ...
    public static void initializeTitanTimer() {
        try (Connection con = ModSupportDb.getModSupportDb()) {
            if (ModSupportDb.hasTable(con, timers)) {
                try (PreparedStatement ps = con.prepareStatement(String.format("SELECT * FROM %s WHERE ID = \"TITAN\"", timers))) {
                    ResultSet rs = ps.executeQuery();
                    Titans.lastSpawnedTitan = rs.getLong("TIMER");
                }
            }
        } catch (SQLException e) {
            RequiemLogging.logException("[Error] in initializeTitanTimer in DbChanges", e);
            throw new RuntimeException(e);
        }

        RequiemLogging.logInfo(String.format("Initialized Titan timer: %d", Titans.lastSpawnedTitan));
        Titans.initializedTitans = true;
    }

    public static void updateLastSpawnedTitan() {
        try (Connection con = ModSupportDb.getModSupportDb()) {
            if (ModSupportDb.hasTable(con, timers)) {
                try (PreparedStatement ps = con.prepareStatement(String.format("UPDATE %s SET TIMER = %%s WHERE ID= \"TITAN\"", timers))) {
                    ps.executeUpdate();
                }
            }
        } catch (SQLException e) {
            RequiemLogging.logException("[Error] in updateLastSpawnedTitan in DbChanges", e);
            throw new RuntimeException(e);
        }
    }
