...
public class KeyEvent {
    ...
    public static void handlePlayerMessage(Message message) {
        if (performer == message.getSender()) {
            ...
            //Requiem.debug(String.format("Current timer: %s", currentTime));
            for (Response r : responses) {
                //Requiem.debug(String.format("Checking if index %s is valid at time %s (%s to %s)", r.index, currentTime, r.startTime, r.endTime));
                if (r.startTime < currentTime && r.endTime > currentTime) {
                    ...
                    //Requiem.debug(String.format("Response at index %s is valid (%s to %s)", r.index, r.startTime, r.endTime));
