...
public class EconomicChanges {
    ...
    public static int getNewValue(Item item) {
        if (item.getTemplateId() == CustomItems.sealedMapId) {
    ...
    public static long getNewShopDiff(Trade trade, long money, long shopDiff) {
        ...
        if (shop == null) {
            RequiemLogging.debug("Something went horribly wrong and the shop is null.");
            return 0;
        }
        RequiemLogging.debug("Money = " + money + ", shopDiff = " + shopDiff);
        if (!shop.isPersonal() && money > 0) {
            RequiemLogging.debug("We're adding money. Testing to see how much difference there is.");
            if (money + shopDiff > 0) {
                RequiemLogging.debug("Player actually purchased something. Reducing the income.");
                long newDiff = money + shopDiff;
                RequiemLogging.debug("Actual difference in currency: " + Economy.getEconomy().getChangeFor(newDiff).getChangeString());
                newDiff *= 0.2;
                RequiemLogging.debug("After 80% void: " + Economy.getEconomy().getChangeFor(newDiff).getChangeString());
                RequiemLogging.debug("Returning the following amount of money to incur the change: " + (-shopDiff + newDiff));
