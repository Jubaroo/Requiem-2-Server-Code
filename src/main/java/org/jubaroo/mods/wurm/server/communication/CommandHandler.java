    ...
    public static boolean onPlayerMessage(Communicator communicator, String msg) {
        ...
        if (msg.startsWith("#setheight ")) {
            if (performer.getPower() < MiscConstants.POWER_IMPLEMENTOR) {
                RequiemLogging.logWarning(String.format("Player %s tried to use #terraform command", performer.getName()));
            } else {
                try {
                    ...
                    if (tokens.hasMoreTokens()) {
                        ...
                        if (height > Short.MAX_VALUE || height < Short.MIN_VALUE) {
                            ...
                            RequiemLogging.logInfo(String.format("#setheight at %d,%d by %s, h=%d r=%d", performer.getTileX(), performer.getTileY(), performer.getName(), height, radius));
                    ...
                    communicator.sendAlertServerMessage(String.format("Error: %s", e.toString()));
                }
            }
        }
        return false;
