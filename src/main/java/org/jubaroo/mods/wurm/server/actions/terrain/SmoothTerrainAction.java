package org.jubaroo.mods.wurm.server.actions.terrain;
...
public class SmoothTerrainAction implements ModAction {
            ...
            for (int x = 1; x >= -1; --x) {
                for (int y = 1; y >= -1; --y) {
                    ...
                    if (x == xMinDiff && y == yMinDiff) {
                        ...
                        //Requiem.debug("Tile " + (tilex + x) + ", " + (tiley + y) + " now at " + newTileHeight + ", rock at " + rockHeight + ".");
    ...
    /*static boolean isTreeGrassBushTile(byte type) {
