...
public class OnPlayerLogin {
    ...
    public static void onPlayerLogin(Player player) {
        try {
            ...
            if (Misc.isRequiemPatreon(player) && DbHelper.canReceiveMonthlyGift(player, "PATREON")) {
                try {
                    if (Misc.isRequiemPatreon(player)) {
                        ...
                        player.getCommunicator().sendServerMessage("A shadowy figure appears behind you and your pocket gets heavier. You quickly turn around to see what is going on only to find that nobody is there. " +
                                "Your monthly sleep powder had been deposited into your inventory!", 255, 192, 203);
                    }
            ...
            loginAnnouncement(player);
            onLogin(player);
            //OnPlayerLogin.addBuffsToPlayersOnLogin(player);
            addMonthlyBuffsToPlayers(player);
