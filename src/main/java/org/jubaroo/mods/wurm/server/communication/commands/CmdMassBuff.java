...
public class CmdMassBuff extends WurmCmd {
    // TODO make this command buff all players until they die or logout. The buff will be determined by the number or phrase that is typed in after the command #massbuff
    public CmdMassBuff() {
        super("#massbuff", MiscConstants.POWER_IMPLEMENTOR);
    }
    ...
    public static void massRefresh(Creature actor) {
        ...
        try {
            ...
            if (nums > allPlayers.length) {
                RequiemLogging.logInfo(String.format("Player %s used %s", actor.getName(), CmdMassBuff.class.getName()));
