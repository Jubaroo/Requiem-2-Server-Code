...
public class PortalMod {
    ...
    public static void onServerStarted() {
        try (Connection con = ModSupportDb.getModSupportDb()) {
            ...
            RequiemLogging.logException("Error creating RequiemPortals table in ModSupport.db", e);
            throw new RuntimeException(e);
        }

        try (Connection con = ModSupportDb.getModSupportDb()) {
            if (!ModSupportDb.hasTable(con, "RequiemGMPortals")) {
                String sql = "CREATE TABLE RequiemGMPortals (\t\tname\t\t\t\t\tVARCHAR(20)\t\tNOT NULL DEFAULT 'Unknown',"
                        + "\t\tposx\t\t\t\t\tINT\t\tNOT NULL DEFAULT 100,"
                        + "\t\tposy\t\t\t\t\tINT\t\tNOT NULL DEFAULT 100,"
                        + "\t\tbank\t\t\t\t\tINT\t\tNOT NULL DEFAULT 0,"
                        + "\t\titemid\t\t\t\t\tLONG\t\tNOT NULL DEFAULT 0)";
            ...
            RequiemLogging.logException("Error creating RequiemGMPortals table in ModSupport.db", e);
            throw new RuntimeException(e);
        }
    }
