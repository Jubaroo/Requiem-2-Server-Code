...
public class LeaderboardCustomQuestion extends Question {
    ...
    protected void topPlayerStats(String statName, int limit) {
        ...
        try {
            dbcon = ModSupportDb.getModSupportDb();
            ps = dbcon.prepareStatement(String.format("SELECT name, %s FROM PlayerStats ORDER BY %s DESC LIMIT %d", statName.toUpperCase(), statName.toUpperCase(), limit));
            rs = ps.executeQuery();
            while (rs.next()) {
                name = rs.getString(1);
                stat = rs.getDouble(2);
    ...
    public void sendQuestion() {
        ...
        f.addBoldText(String.format("Top %d players in %s", limit, this.getQuestion()));
        f.addText("\n\n");
        int i = 0;
